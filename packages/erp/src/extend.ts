...
/**
 * Bind an ORM model to a pack resource base (most common app wiring).
 *
 * @example
 * ```ts
 ...
 * Prefer this over copy-pasting resource files.
 *
 * @example
 * ```ts
 ...
 *   model: ProductEntity,
 *   form: (schema) => {
 *     schema.section('custom', 'Custom').fields(TextField.make('brand'));
 *   },
 * });
 * ```
 */
...
/**
 * Wrap a domain store so you can add behavior (hooks, logging, extra tables)
 * without rewriting the ORM adapter.
 */
