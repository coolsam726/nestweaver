...
Checklist to call **`@nodeweaver/loom` 1.0 — production stable**.

Target: [milestone](https://github.com/coolsam726/nodeweaver/milestone/1) (~2026-09-06).

## Product stance

- **Nodeweaver** scaffolds NestJS + frontend monorepos.
...
| CI runs Loom unit tests (`pnpm test` → `@nodeweaver/loom test`) | Done |
...
| npm publish `@nodeweaver/loom` + `create-nodeweaver` | Pending first GitHub Release |
| Deprecation freeze for 1.0 (no silent removals) | Pending — warn-only until 2.0 |

## Release steps

1. Confirm `pnpm test` green on `main`.
2. Bump versions in `packages/loom`, `packages/nodeweaver`, `packages/create-nodeweaver` (keep in sync).
3. Tag `v1.0.0` and create a GitHub Release (triggers Publish workflow).
4. Verify `npm create nodeweaver@latest` boots `/admin` (Loom is always scaffolded).
5. Announce: Loom is the NestJS admin; Nodeweaver is how you start.
